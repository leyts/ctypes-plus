...
# `_CData`, the direct base of every ctypes type; not publicly importable.
...
def _ctype(annotation: object) -> type:
    ...
    if get_origin(annotation) is Annotated:
        for meta in get_args(annotation)[1:]:
            if isinstance(meta, type) and issubclass(meta, _CDATA):
                return meta
...
def build(cls: type, base: type[Structure | Union]) -> type:
    ...
    namespace["_fields_"] = [
        (name, _ctype(ann)) for name, ann in annotations.items()
    ]
